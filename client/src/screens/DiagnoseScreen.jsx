  ...
  return (
    <div>
      <div className="page-header">
        <div className="text-xxl" style={{ fontWeight: 800 }}>{headline}</div>
        <div className="mt-12" style={{ position: "relative" }}>
          <MdSearch style={{ position: "absolute", left: 12, top: 12, color: "rgba(255,255,255,0.9)" }} />
          <input
            value={query}
      ...
      <button
        ...
        onClick={() => window.alert("Cloud image scan integration will be connected in the next milestone.")}
      >
        <MdCameraAlt size={20} />
        Scan Your Crop
      </button>
      ...
      <div className="card mt-12" style={{ display: "flex", gap: 8 }}>
        ...
        <div className="text-md" style={{ color: "#1b5e20", lineHeight: 1.35 }}>
          ...
          <br />
          Aphid activity is high in nearby farms. Inspect mustard and wheat fields today.
        </div>
      </div>

      <div className="text-xl mt-16" style={{ fontWeight: 800 }}>Common Diseases</div>
      <div className="mt-10" style={{ display: "grid", gap: 10 }}>
        {diseaseList.map((item) => (
          <div className="card" key={`${item.name}-${item.crop}`}>
            <div className="row-between">
              <div className="text-lg" style={{ fontWeight: 700 }}>{item.name}</div>
              <div className="text-sm" style={{ fontWeight: 700, color: severityColor(item.severity) }}>{item.severity}</div>
            </div>
            <div className="text-sm mt-8 muted" style={{ fontWeight: 600 }}>Crop: {item.crop}</div>
      ...
      {recent.length > 0 && (
        <>
          <div className="text-lg mt-16" style={{ fontWeight: 800 }}>Recent AI Diagnoses</div>
          <div className="mt-8" style={{ display: "grid", gap: 8 }}>
            {recent.map((item) => (
              <div className="card" key={item._id} style={{ display: "flex", gap: 10, alignItems: "center" }}>
