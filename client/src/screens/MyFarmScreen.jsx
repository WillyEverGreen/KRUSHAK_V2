...
export default function MyFarmScreen() {
  ...
  useEffect(() => {
    if (data?.reminders) {
      setReminders(
        data.reminders.map((item) => ({ ...item, done: Boolean(item.done) })),
      );
    }
  }, [data]);
  ...
  function addLocalReminder() {
    ...
    if (!task) return;
    const dueAtLabel =
      window.prompt("Time label", "Tomorrow 6:00 AM") || "Tomorrow";
  ...
  return (
    <div>
      ...
      <div className="tab-strip mt-14" style={{ marginTop: 14 }}>
        {["All Crops", `Reminders (${reminderCount})`, "Livestock"].map(
          (label, index) => (
            <button
              key={label}
            ...
            <div className="card" key={crop.name}>
              ...
              <div className="row mt-10" style={{ alignItems: "center" }}>
                <div className="health-bar" style={{ flex: 1 }}>
                  <span
                    style={{ width: `${Math.round(crop.health * 100)}%` }}
                  />
                </div>
              ...
              <div className="text-sm muted mt-10">
                Water: {crop.water} - {crop.action}
              </div>
            </div>
          ))}
              ...
              <div className="row-between">
                ...
                <button
                  ...
                  onClick={() =>
                    setReminders((prev) =>
                      prev.map((r) =>
                        r._id === item._id ? { ...r, done: !r.done } : r,
                      ),
                    )
                  }
                >
        ...
        {activeTab === 2 &&
          livestockTips.map((item) => (
            <div className="card" key={item.title}>
              ...
              <div className="text-sm muted mt-8" style={{ lineHeight: 1.35 }}>
                {item.tip}
              </div>
            </div>
          ))}
